...
public class ReminderAirDrop extends CustomRel {
    List<ReminderData> listReminder;
    FragReminder reminder;
    public ReminderAirDrop(Context context, List<ReminderData> listReminder) {
        super(context, R.layout.frag_reminder_airdropchild);
    ...
    public void onStart(FragReminder fragReminder) {
        ...
        findViewById(R.id.newReminder).setOnClickListener(v -> {
            ...
            MainActivity.getGlobal().TypeOfReminder="airdrop";
            ...
            if(reminder.totalSize==reminder.freeCount && reminder.isVip==-1) {
                ...
                if (!mLocalData.isFirstReminder(getContext())) {
                    if (Build.VERSION.SDK_INT > 28 && !Settings.canDrawOverlays(getContext())) {
                        ...
                        MainActivity.getGlobal().FinishFragStartFrag(new FragGetReminderInfo().getInsertInstance("airdrop"));
                        //MainActivity.getGlobal().MainDiallog("reminderDialog" , 1 , null , false,-1);
                    }
