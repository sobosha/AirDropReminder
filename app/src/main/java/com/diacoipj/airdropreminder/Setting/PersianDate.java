...
//		return ((Year % 4) == 0);
	...
	public int[] toJalaliWithOneString(String date) {
			int year = Integer.parseInt(date.split(date.contains("-") ? "-" : "/")[0]);
			int month = Integer.parseInt(date.split(date.contains("-") ? "-" : "/")[1]);
			int day = Integer.parseInt(date.split(date.contains("-") ? "-" : "/")[2]);
	/*	if(!Setting.isEnLang()) {*/
			int hshDay = 0;
			int hshMonth = 0;
			int hshElapsed;
